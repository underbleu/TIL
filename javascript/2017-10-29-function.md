...
보통 함수명 생략한다. 함수변수가 함수명처럼 사용되기 때문  
    * 익명함수표현식: 변수로 익명함수를 호출
    * 기명함수표현식: 변수로 함수호출. 함수명은 외부접근 불가. 재귀적호출등의 용도로만 씀
* **세미콜론;을 붙이도록 강력히 권장**  
...
자바스크립트에서는 함수도 객체다. 함수는 기본기능인 코드 실행뿐만 아니라 일반객체처럼 프로퍼티들을 가질 수 있다. 자바스크립트에서 함수는 일반 객체처럼 취급될 수 있다.
...
자바스크립트에서 함수는 일급객체이다. 따라서 함수는 기본기능인 코드 실행뿐만 아니라 위와 같은 동작을 할 수 있다. (C에서 함수는 이급객체이다)
...
```
-> 객체형인수를 전달하여 원본객체가 변경되는 side-effect가 발생했다. 이런 부수효과를 발생시키는 함수를 비순수함수(Impure function)이라 한다. 이는 복잡성을 증가시켜 디버깅을 어렵게 만든다. 비순수함수를 최대한 줄이도록! 
...
* 배열과 비슷하게 행동하지만, 실제 배열은 아니기 때문에 배열메서드 사용하면 에러가 난다(apply call등을 써야함)
...
* `foo.prototype.constructor`는 `foo`를 가르킴
...
>ECMAScript에서는`[[Prototype]]`로  
크롬브라우저에서는  `__proto__`로 명시  
결론적으로 두개는 상황에따라 다르게 표기할뿐 **같은개념**이다. 
...
-> 자바스크립트는 Function Level Scope 지원. 함수안에 있는 변수들은 함수안에서만 유효범위를 가진다(유의: var로 선언안하면 전역Scope됨). 따라서 IIFE안에 라이브러리 코드를 추가하면 전역 Name Space를 더럽히지 않으므로 변수이름 충돌 같은 문제를 방지할 수 있다

### 3. 내부함수(Inner Function)
함수 내부에 정의된 함수를 의미한다. 클로저생성, 외부에서 접근을 막는 독립적인 헬퍼함수를 만들 때 사용한다.
1. 내부함수에서 외부함수의 변수에 접근이 가능하다
2. 외부함수에서 내부함수의 변수에 접근은 불가능하다
3. 일반적으로 내부함수는 부모함수 내에서만 호출가능하다(Function-Level-Scope)
4. 내부함수를 외부함수의 리턴값으로 받으면, 함수 밖에서도 내부함수 호출가능  
...
//4.리턴받은 내부함수는 밖에서도 호출OK 
var inner2 = outter(); //1 3
inner2(); //2 4
```
-> 5. 변수 inner2에 outter();함수를 실행시키면서 담는다. inner()에 있던 1 3을 뱉어내고, inner2()가 내보낸 b, c만을 리턴값으로 받아 변수에 저장한다. 이 때 inner2();는 실행이 종료된 외부함수 outter()의 스코프에 있는 변수b의 값 '2'를 참조하는 **클로저**이다 
